...
export default function PrioritiesScroll() {
 ...
 // --- GRADIENTS & OPACITIES BASED ON SCROLL POSITION ---
  ...
  const creativityGradient = useTransform(scrollYProgress, 
    [0, 0.5, 1], 
    [
  ...
  const creativityOpacity = useTransform(scrollYProgress, [0, 0.5, 1], [1, 0.5, 0]);

  // Reasonableness Focus Timeline
  const reasonablenessGradient = useTransform(scrollYProgress, 
    [0, 0.5, 1], 
    [
  ...
  const reasonablenessOpacity = useTransform(scrollYProgress, [0, 0.5, 1], [0.4, 1, 0.4]);

  // Speed Focus Timeline
  const speedGradient = useTransform(scrollYProgress, 
    [0, 0.5, 1], 
    [
      "linear-gradient(to bottom, rgba(255,255,255,0) 0%, rgba(255,255,255,0) 20%, rgba(255,255,255,0) 80%, rgba(255,255,255,0) 100%)",
      "linear-gradient(to bottom, rgba(255,255,255,0.4) 0%, rgba(255,255,255,0.4) 20%, rgba(255,255,255,0) 80%, rgba(255,255,255,0) 100%)", 
  ...
  const desc1Opacity = useTransform(scrollYProgress, (latest) => (latest < 0.33 ? 1 : 0));
  const desc2Opacity = useTransform(scrollYProgress, (latest) => (latest >= 0.33 && latest < 0.66 ? 1 : 0));
  const desc3Opacity = useTransform(scrollYProgress, (latest) => (latest >= 0.66 ? 1 : 0));
        ...
        <div className="relative z-20 w-full max-w-[1200px] mx-auto px-6 md:px-8 flex flex-col md:flex-row gap-10 md:gap-24 items-start">
          ...
          <div className="flex flex-col items-start h-[450px] md:w-2/3 justify-start relative pt-10">
            ...
            <div className="relative min-h-[120px] w-full mt-16 md:mt-24">
              ...
              >
               Hacemos un trabajo rápido y de calidad. Manteniendo el enfoque en cada detalle.
              </motion.p>
              ...
              <motion.p
                ...
                className="text-base md:text-xl tracking-wide font-light max-w-[500px] absolute top-0 left-0 transition-opacity duration-200"
              >
                Our priority is to do the project quickly, but at the same time pay attention to the quality final result.
              </motion.p>
            </div>
